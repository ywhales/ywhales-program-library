use crate::*;
use ywpl_token_vault::instruction;
...
impl ExternalPrice {
    ...
    ) -> ywpl_token_vault::state::ExternalPriceAccount {
    ...
    pub async fn create(&self, context: &mut ProgramTestContext) -> transport::Result<()> {
        ...
        let tx = Transaction::new_signed_with_payer(
            &[system_instruction::create_account(
                &context.payer.pubkey(),
                &self.keypair.pubkey(),
                rent.minimum_balance(ywpl_token_vault::state::MAX_EXTERNAL_ACCOUNT_SIZE),
                ywpl_token_vault::state::MAX_EXTERNAL_ACCOUNT_SIZE as u64,
                &ywpl_token_vault::id(),
            )],
            Some(&context.payer.pubkey()),
            &[&context.payer, &self.keypair],
